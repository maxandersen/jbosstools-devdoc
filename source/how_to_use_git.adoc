= How to use Git with JBoss Tools
...
=== Create a dir for storing your jbosstools projects

JBoss Tools consists of many different repositories. We recommend you put all the repositories under one directory for simpler navigation.

In this document we use `~/jbosstools` as the root location for the JBoss Tools repositories, but you can of course use your own favorite location.
...
https://hub.github.com/[hub] is optional to install but severely reduces the amount of commands needed to work
with github fork and pull-request model. In this doc we'll have a both "with hub" and "plain git" instructions,
but as you'll see the `hub` commands are generally more concise and effective to use.

== Sources

=== How to clone a repo to fetch sources

To fetch sources, you  first clone the relevant repo to your local disk. Most of JBoss Tools repositories are available from https://github.com/jbosstools[jbosstools organization] on github.

In the following we just use `jbosstools-base` as an example.
...
=== Which remote to use ? http, http, ssh ?

Github provides mulitple protocols for its repositories, http, https, git and git+shh. The list have grown over the years and question is which one to use ?

The short answer is `https`.

The longer answer is:

. you can copy/paste the http url straight from the browser url (no need to fiddle with the git/ssh UI on github.com)
. git hub uses https as default when using fork etc. so it fits nicely with that
. it does not require ssh agent running to remember passwords and http://stackoverflow.com/questions/5343068/is-there-a-way-to-skip-password-typing-when-using-https-github[git provides credential.helper] for more portable setup.
. it is the https://help.github.com/articles/which-remote-url-should-i-use/[recommended way from github] with the following reasons:
.. https works everywhere even behind proxy
.. https is available on all repos (public or private)
.. https automatically knows if you have read, read/write based on your permissions (same setup for committers as well as contributors)
